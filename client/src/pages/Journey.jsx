      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className="space-y-24">
            {milestones.map(function (milestone, index) { return (<motion.div key={index} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} className={"flex flex-col md:flex-row gap-8 items-center ".concat(index % 2 === 1 ? "md:flex-row-reverse" : "")}>
                {/* Image side */}
                <div className="w-full md:w-1/2 relative">
                  <div className="absolute top-1/2 -translate-y-1/2 right-[-2.5rem] w-10 h-10 bg-accent rounded-full border-4 border-background z-10 hidden md:flex items-center justify-center translate-x-1/2">
                    {index % 2 === 0 && (<milestone.icon size={18} className="text-accent-foreground"/>)}
                  </div>
                  <div className="absolute top-1/2 -translate-y-1/2 left-[-2.5rem] w-10 h-10 bg-accent rounded-full border-4 border-background z-10 hidden md:flex items-center justify-center -translate-x-1/2">
                    {index % 2 === 1 && (<milestone.icon size={18} className="text-accent-foreground"/>)}
                  </div>
                ...
                {/* Content side */}
                <div className={"w-full md:w-1/2 flex flex-col justify-center ".concat(index % 2 === 0 ? "md:pl-12" : "md:pr-12")}>
