...
const teamMembers: { name: string; role: string; image: string; bio: string; imgClass?: string }[] = [
  {
        ...
        <div className="flex flex-col gap-8">
          {teamMembers.map((member, idx) => (
            <motion.div
              ...
              className="flex flex-col sm:flex-row gap-6 bg-card border border-border/50 rounded-2xl p-6 sm:p-8 shadow-md shadow-primary/5"
            >
              {/* Square image */}
              <div className="shrink-0 mx-auto sm:mx-0">
                <img
                  src={member.image}
                  alt={member.name}
                  className={`w-44 h-44 sm:w-52 sm:h-52 rounded-xl object-cover shadow-md ${member.imgClass ?? "object-center"}`}
                />
              </div>
